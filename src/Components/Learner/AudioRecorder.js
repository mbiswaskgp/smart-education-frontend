...
class AudiRecorder extends React.Component {
    ...
    uploadRecording = () => {
        // var 
      console.log(this.props.questionDetailId);
      //console.log(this.state.blobData.result);
      let formData = new FormData();
      this.setState({ isUploadOn: false });

      formData.append("file", this.state.blobData.result);
      formData.append("questionDetailId", this.props.questionDetailId);
      CommonService.updatePostImage('saveLearnerAudio',20, formData)
      .then(response => {
          //setSubmitted(false);
          ...
          if(response.data.success){
            ...
            this.props.parentCallback(response.data.data.audioPath);
          }else{
