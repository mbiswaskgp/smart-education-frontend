...
function TutorInactiveList() {
    ...
    useEffect(() => {
        retrieveAllTutors(limit, offset, sort, searchTitle,searchCond);
        dispatch(changeCurrentPage('adminTutor'));
    }, []);
    
    const retrieveAllTutors = (limit, offset, sort, searchTitle,searchCond) => {
        setLoader(true);
        CommonService.getAllWithPage('tutors/search',{ limit, offset, sort, searchTitle,searchCond })
        .then(response => {
    ...
    const findByTitle = (e) =>{
        var offset1 = 0;
        retrieveAllTutors(limit, offset1, sort, searchTitle,searchCond );
    }

    const handleSaveToActive = (id) => {
       
        swal({
            title: "Are you sure you want to activate this tutor?",
            text: "Once activated, the tutor will go on the active list!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
        .then((willDelete) => {
            if (willDelete) {
                var formData = {};
                CommonService.updatePost('saveTutorToActive',id, formData)
                    .then(response => {
    ...
    const onPageChanged = (page) => {
        ...
        setActivePage(page);
        retrieveAllTutors(limit, pageOffset, sort, searchTitle,searchCond);
    }
    ...
    return (
        <div>
                                    ...
                                    <tr key={tutor.id}>
                                        ...
                                        <td>
                                            ...
                                            {(loaderArr1?.id==tutor.id)?
                                                ...
                                                :
                                                <button className="iconButton2" onClick={() => handleSaveToActive(tutor.id)}><span>Active</span></button>
                                            }
                                        </td>
                                    </tr>
                                ))}                      
            ...
            <div className="rows">
                ...
                <div className="col-md-4 col-4">
                    <Link to="/admin/tutor"> Active Tutors <i className="fa fa-arrow-right" aria-hidden="true"></i> </Link>
                </div>
            </div>
        </div>
    );
...
export default TutorInactiveList;